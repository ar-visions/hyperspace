...
layout(location = 4) in vec3  v_eye_dir;
...
void main() {
    vec4 color  = texture(tx_color,  v_uv0).rgba;
    vec3 nmap   = texture(tx_normal, v_uv0).rgb * 2.0 - 1.0;
    ...
    pixel = vec4(1.0, 1.0, 1.0, 1.0); //color;//vec4(v_uv.x, v_uv.y, 0.0, 1.0);
}
