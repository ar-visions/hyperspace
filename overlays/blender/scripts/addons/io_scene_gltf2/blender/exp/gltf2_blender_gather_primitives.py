...
def gather_primitives(
    ...
    for internal_primitive in blender_primitives:
        ...
        if not export_settings['gltf_quads']:
            for face in internal_primitive['faces']:
                if len(face) == 4:  # Check if it's a triangle
                    raise ValueError("mesh expected to contain only quads.")
