...
class PrimitiveCreator:
    ...
    def primitive_split(self):
        ...
        gltf_quads = self.export_settings['gltf_quads']
        try:
            if not gltf_quads:
                ...
                print('trying to split quads from quads...')
                loop_indices = np.empty(len(self.blender_mesh.polygons) * 4, dtype=np.uint32)
                self.blender_mesh.polygons.foreach_get('vertices', loop_indices)
        ...
        if self.use_materials == "NONE": # Only for None. For placeholder and export, keep primitives
            ...
            loop_material_idxs = np.repeat(tri_material_idxs, 4 if use_quads else 3)  # material index for every loop
    ...
    def primitive_creation(self):
        ...
        for material_idx, dot_indices in self.prim_indices.items():
            ...
            primitives.append({
                ...
                'mode': 7 if self.export_settings['gltf_quads'] else 4,
